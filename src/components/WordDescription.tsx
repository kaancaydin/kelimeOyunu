...
const Clues = ({ children, color }: CluesProps) => {
  return (
    <div className={`items-center px-1.5 py-0.5  sm:px-3 sm:py-1.5 rounded-full
        transition-all overflow-hidden ${color}  
        bg-linear-to-r from-transparent to-white/5 
        ring-1 ring-offset-1 ring-offset-gray-900
        capitalize font-medium text-gray-200 text-sm sm:text-base`}>
        {children}
    </div>
  );
};
  ...
  return (
    <div className="relative w-full px-4 sm:px-10 py-3 sm:py-6 text-center cursor-default select-none italic text-gray-200">
      ...
      <div className="w-full flex items-center justify-center px-2 sm:px-6 min-h-20 sm:min-h-37.5">
        <p className="text-lg sm:text-4xl font-medium leading-relaxed tracking-tight text-center italic text-gray-200 wrap-break-word">
      ...
      <div className="flex justify-center items-center gap-6 mt-2">
        ...
        <Clues color="ring-amber-400"> {`${aktifKelime.kelimeSayisi} Kelime`} </Clues>
      </div>
    </div>
  );
};
